...
const SocialShare: React.FC<SocialShareProps> = ({
  ...
  const shareOnFacebook = () => {
    const shareUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}&quote=${encodedDescription}`;
    openShareWindow(shareUrl);
  };

  const shareOnTwitter = () => {
    const shareUrl = `https://twitter.com/intent/tweet?url=${encodedUrl}&text=${encodedDescription}`;
    openShareWindow(shareUrl);
  };
  ...
  const shareOnWhatsApp = () => {
    const shareUrl = `https://api.whatsapp.com/send?text=${encodedDescription}%20${encodedUrl}`;
